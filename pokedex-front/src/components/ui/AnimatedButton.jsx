...
// Recebendo as props necessárias para a paginação
const AnimatedButton = ({ onClick, disabled, isLeft }) => {
  return (
    // Passamos as props para o CSS tratar a opacidade e a rotação
    <StyledWrapper $disabled={disabled} $isLeft={isLeft}>
      <div className="btn-conteiner">
        {/* Transformado em <button> real */}
        <button 
            onClick={onClick} 
            disabled={disabled} 
...
const StyledWrapper = styled.div`
  ...
  .btn-content {
    ...
    /* Controle do Disabled e Rotação da prop isLeft */
    cursor: ${({ $disabled }) => ($disabled ? 'not-allowed' : 'pointer')};
    opacity: ${({ $disabled }) => ($disabled ? 0.3 : 1)};
    transform: ${({ $isLeft }) => ($isLeft ? 'rotate(180deg)' : 'none')};
  }

  /* Só aplica animações se não estiver disabled */
  ${({ $disabled }) => !$disabled && `
    .btn-content:hover,
    .btn-content:focus {
      transition: 0.5s;
